...
void ABlasterPlayerController::InitTeamScores()
{
	...
	if (bHUDValid)
	{
		...
		BlasterHUD->CharacterOverlay->ScoreSpacerText->SetText(FText::FromString(Spaces));
	}
}
...
void ABlasterPlayerController::CheckPing(float DeltaTime)
{
	...
	if (HighPingRunningTime > CheckPingFrequency)
	{
		...
		if (PlayerState)
		{
			if (PlayerState->GetPing() * 4 > HighPingThreshold) //ping is compressed, it's actualy ping / 4
			{
				HighPingWarning();
...
void ABlasterPlayerController::HighPingWarning()
{
	...
	bool bHUDValid = BlasterHUD
		&& BlasterHUD->CharacterOverlay
		&& BlasterHUD->CharacterOverlay->WifiImage
		&& BlasterHUD->CharacterOverlay->HighPingAnimation;
	if (bHUDValid)
	{
		BlasterHUD->CharacterOverlay->WifiImage->SetOpacity(1.f);
...
void ABlasterPlayerController::StopHighPingWarning()
{
	...
	if (bHUDValid)
	{
		BlasterHUD->CharacterOverlay->WifiImage->SetOpacity(0.f);
...
void ABlasterPlayerController::SetHUDTime()
{
	...
	if (CountdownInt != SecondsLeft)
	{
		...
		{
			SetHUDAnnounementCountdown(TimeLeft);
		}
...
void ABlasterPlayerController::OnPossess(APawn* InPawn)
{
	...
	if (BlasterCharacter)
	{
		...
		SetHUDWeaponAmmo(BlasterCharacter->GetWeaponAmmo());
...
void ABlasterPlayerController::SetHUDAnnounementCountdown(float CountdownTime)
{
	BlasterHUD = BlasterHUD == nullptr ? Cast<ABlasterHUD>(GetHUD()) : BlasterHUD;
	bool bHUDValid = BlasterHUD
...
FString ABlasterPlayerController::GetTeamsInfoText(ABlasterGameState* BlasterGameState)
{
	...
	{
		InfoTextString = FString::Printf(TEXT("%s\n"), *Announcement::TeamsTideForTheWin);
...
void ABlasterPlayerController::ClinetJoinMidgame_Implementation(FName StateOfMatch, float Warmup, float Match, float Cooldown, float StartingTime)
{
	WarmupTime = Warmup;
	MatchTime = Match;
...
void ABlasterPlayerController::ServerCheckMatchState_Implementation()
{
	...
	if (GameMode)
	{
		...
		ClinetJoinMidgame(MatchState, WarmupTime, MatchTime, CooldownTime, LevelStartingTime);
	}
}
